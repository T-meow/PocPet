...
const preparedStarter = craftRecipe(starter, 'plain_rice', false, 1, 'starter-rice', now);
let cooked = craftRecipe(preparedStarter, 'egg_rice', false, 1, 'first', now);
assert.equal(cooked.hearts, 1);
assert.equal(cooked.inventory.dish_egg_rice, 1);
assert.equal(cooked.inventory.rice ?? 0, 0);
assert.equal(cooked.partnerSchedule.skills.cooking.xp - preparedStarter.partnerSchedule.skills.cooking.xp, 6, 'the first recipe grants one practice XP plus the existing five-point bonus');
...
for (const [level, basic, skilled, master] of [[1, 1, 1, 2], [20, 1, 1, 2], [99, 1, 1, 2]]) {
  for (const [skillLevel, expected] of [[1, basic], [5, skilled], [10, master]]) {
    ...
    assert.equal(getKitchenHeartReward(pet, 'egg_rice').heartsPerServing, expected);
    const made = craftRecipe(pet, 'egg_rice', false, 2, `reward-${level}-${skillLevel}`, now);
    assert.equal(made.hearts - pet.hearts, expected * 2, 'cooking hearts depend on the recipe and cooking skill, not pet level');
...
assert.equal(thresholdMade.hearts, getKitchenHeartReward(thresholdPet, 'egg_rice').heartsPerServing * 5);
...
assert.equal(riceMade.hearts, 0);
...
assert.deepEqual(cake.inventory, { emergency_biscuit: 21, egg: 1, dish_biscuit_layer_cake: 2 });
assert.equal(craftRecipe(cakePet, 'biscuit_layer_cake', false, 3, 'too-many-cakes', now), cakePet);
const insufficientCakePet = { ...cakePet, inventory: { emergency_biscuit: 9, strawberry_milk: 2, egg: 1 } };
...
for (const [method, action] of [['mix', 'stir'], ['pan', 'flip'], ['blender', 'blend'], ['oven', 'bake']] as const) {
  ...
  for (let index = 0; index < 3; index++) {
    ...
    assert.equal(dish.partnerSchedule.skills.cooking.xp - preparedStarter.partnerSchedule.skills.cooking.xp, index === 2 ? 6 : 0, 'cooking practice is awarded only after the final action');
  }
...
try {
  ...
  assert.ok(renderStorageActions(ShopModal, shopProps).includes('购买 5 件'));
  ...
  const boxHtml = renderStorageActions(ShopModal, boxProps);
  assert.ok(boxHtml.includes('购买 3 箱') && boxHtml.includes('到账 120 块苏打饼干'));
  const inventoryActions = renderStorageActions(InventoryModal, inventoryProps);
  ...
  assert.ok(!searchedHtml.includes('没有找到符合的物品') && searchedHtml.includes('data-item-id='), 'hidden search must not filter the shop by an old query');
  const claimHtml = renderStorageActions(ShopModal, { ...shopProps, pet: claimedBiscuits, browse: { ...defaultBrowse, selectedId: 'emergency_biscuit', quantity: 10 } });
  ...
  const readingHtml = renderToStaticMarkup(createElement(ItemRecoveryPreview, readingProps));
  assert.ok(readingHtml.includes('学习经验 +3') && readingHtml.includes('其中 4 本在满级后阅读') && readingHtml.includes('基础心心 +4'));
  const masteredPet = structuredClone(readingPet); masteredPet.partnerSchedule.skills.study.level = 10;
  ...
  const achievementProps = { pet: supplementPet, activeCategory: 'all', itemIconMap: assets.itemIcons, onBack: noop, onCategoryChange: noop, onClaimReward: noop, onClaimAllRewards: noop };
  const suppliesHtml = renderToStaticMarkup(createElement(AchievementsPage, achievementProps));
  ...
  try {
    ...
    assert.ok(englishShop.includes('Ingredients') && renderStorageActions(ShopModal, shopProps).includes('Buy 5'));
    const englishBox = renderStorageActions(ShopModal, boxProps);
    assert.ok(englishBox.includes('Buy 3 boxes') && englishBox.includes('Receive 120 soda biscuits'));
    const englishBagActions = renderStorageActions(InventoryModal, inventoryProps);
    assert.ok(englishBagActions.includes('Feed ×5') && englishBagActions.includes('Cook with it'));
