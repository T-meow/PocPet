...
import { facilityIds, facilityAvailable } from './communityData';
...
export const getAdventureStartReason = (pet: PetState, region?: AdventureDestinationId, now = Date.now(), purpose?: CommunityRoute) => {
  ...
  if (purpose && facilityIds.includes(purpose as FacilityId) && !facilityAvailable(pet, purpose as FacilityId)) return '先完成这项建设的前置设施，再去寻找线索。';
  ...
  const first = getAdventureSteps(5, region, purpose)[0].choices[0];
...
export const advanceAdventure = (pet: PetState, tripId: string, expectedStep: number, choiceId: string, now = Date.now()): PetState => {
  ...
  const choice = getAdventureSteps(trip.rulesVersion, trip.region, trip.purpose)[expectedStep]?.choices.find(value => value.id === choiceId);
  ...
  const found: Inventory = { ...discovery.items, ...(trip.purpose ? {} : trip.region === 'tutorial' ? complete ? { map_handbook: 1, coin_hoard: 1 } : {}
