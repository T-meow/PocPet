...
export interface SpecialtyOrder { id: string; day: string; item: SpecialtyItem; quantity: number; unitPrice: number; multiplier: 3 | 5; acceptedAt: number }
...
export const getSpecialtyCandidates = (pet: PetState, now = Date.now()): SpecialtyOrder[] => {
  ...
  return [0, 1].map(index => {
    ...
    return { id: `specialty:${day}:${item}`, day, item, quantity: data.quantity, unitPrice: data.base * multiplier, multiplier, acceptedAt: 0 };
  });
};
...
export const acceptSpecialtyOrder = (pet: PetState, id: string, now = Date.now()): PetState => {
  ...
  return { ...pet, community: { ...pet.community, specialtyOrders: { ...state, acceptedDay: day, active: { ...order, acceptedAt: now } } }, recentEvent: `已锁定${order.multiplier}倍收购价。交付${specialtyGoods[order.item].name} ×${order.quantity}可得 ${order.quantity * order.unitPrice} 金币；接取后不过期。` };
};
...
export const claimSpecialtyOrder = (pet: PetState, id: string): PetState => {
  ...
  const coins = clampCoins(pet.coins + order.quantity * order.unitPrice) - pet.coins;
