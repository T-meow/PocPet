...
import { isLandmarkId, parseLandmarkId } from './landmarkProgress';
...
export const isAdventureCarryItem = (id: string) => isBuiltinItemId(id) || isAdventureSupply(id);
...
export const defaultAdventureState = (): AdventureState => ({ schemaVersion: 8, landmarks: [], backpackLevel: 0, valleyCompleted: [], starterClaimed: false, starterMealsClaimed: false, tripsStarted: 0, completed: {}, lastCompletedDay: {}, discoveries: [], active: undefined, pending: undefined, journal: [] });
...
const normalizeResult = (raw: unknown): AdventureResult | undefined => {
  ...
  if (value.region === 'hills') value.region = 'windmill';
  if (value.region === 'station') value.region = 'observatory';
  if (!text(value.id) || !(adventureDestinationIds as readonly unknown[]).includes(value.region)) return undefined;
...
export const normalizeAdventureState = (raw: unknown): AdventureState => {
  ...
  return {
    schemaVersion: 8, landmarks: Array.isArray(value.landmarks) ? [...new Set(value.landmarks.filter(isLandmarkId))] : [], backpackLevel: normalizeBackpackLevel(value.backpackLevel), valleyCompleted: Array.isArray(value.valleyCompleted) ? [...new Set(value.valleyCompleted.filter(isValleyQuest))] : [], starterClaimed: value.starterClaimed === true, starterMealsClaimed: value.starterMealsClaimed === true, tripsStarted: count(value.tripsStarted),
    ...
    lastCompletedDay,
    active: pending ? undefined : normalizeTrip(value.active, ![2, 3, 4, 5, 6, 7, 8].includes(Number(value.schemaVersion)), explorationBackpackCapacities[normalizeBackpackLevel(value.backpackLevel)]), pending,
    journal,
  };
};
