import { adventureBagCapacity, getAdventureStepCount } from './adventureData';
...
export const chooseAdventureReturnItems = (pet: PetState, resultId: string, chosen: Inventory): PetState => {
  ...
  if (!result || result.id !== resultId || !result.salvage || result.rewardsClaimed || getAdventureBagCount(chosen) > adventureBagCapacity ||
