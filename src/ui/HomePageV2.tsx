...
import { canClaimBoostCardDailyReward, getDailyWishView, getReturnWelcomeView } from '../core/pet';
...
export const HomePageV2 = (props: Props) => {
  ...
  const quickPlayHint = busy ? L('伙伴正在社区帮忙', 'Your companion is helping in the community')
      ...
      <section>
        ...
        <div className="home-quick-grid home-services-grid">
          ...
          <button className="home-quick schedule" onClick={onOpenPartnerSchedule}>
            <CalendarDays /><strong>{L('社区工作', 'Community work')}</strong><small>{pet.partnerSchedule.pendingResult ? L('报酬待领取', 'Rewards are ready') : busy ? L('正在帮忙', 'Lending a hand') : L('快速工作与邻里事务', 'Quick work and local requests')}</small>
          </button>
