...
export const IdleExpeditionPreparation = ({ pet, actorId, actorName, update, registry, icons, onUseHomeItem, onShop, initialRegion, initialTarget, onMap, onCamp, onRoute }: ExpeditionProps & PreparationResources & {
  initialRegion: RegionId; onMap: (region: RegionId) => void; onCamp: (region: RegionId) => void; onRoute: (region: RegionId, target: ValleyGatherTarget) => void;
}) => {
  const [region, setRegion] = useState(initialRegion);
  const [target, setTarget] = useState<ValleyGatherTarget>(initialTarget && initialTarget !== 'aquamarine' ? initialTarget : 'valley_mushroom');
  ...
  const reason = getExpeditionStartReason(pet, [region], 'idle', hours, Date.now(), 'patrol', rations);
  ...
  return <>
    <div className="idle-preparation-controls">
      <div className="idle-preparation-route">
        <label className="idle-preparation-field"><span>目的地</span><select disabled={busy} aria-label="挂机目的地" value={region} onChange={e => setRegion(e.target.value as RegionId)}>{regionIds.map(id => <option key={id} value={id}>{regions[id].name}{!getRegionUnlocked(pet, id) ? ' · 未解锁' : !pet.community.expedition.regions[id].surveyed ? ' · 需完成故事' : !pet.community.expedition.regions[id].base ? ' · 需建设营地' : ''}</option>)}</select></label>
    ...
    <PreparationInventory pet={pet} registry={registry} icons={icons} bag={rations.food} capacity={q.maximum} automaticFood={q.purchased ? { trail_mix: q.purchased } : {}} foodOnly onPack={changeFood} onUseHomeItem={onUseHomeItem} perform={perform} bagExtra={<>
      {!ready && <div className="outpost-unlock-note"><p>{!unlocked ? regions[region].unlockHint : !progress.surveyed ? '完成当地故事后，就能修好营地并安排挂机。' : '修好当地营地后开放挂机探索。'}</p><button className="exp-secondary" onClick={() => { action.cancel(); progress.surveyed && unlocked ? onCamp(region) : onMap(region); }}>{progress.surveyed && unlocked ? '去建设营地' : '去地图查看'}</button></div>}
