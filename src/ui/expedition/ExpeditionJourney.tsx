...
export const ExpeditionJourney = ({ pet, update, onCommunity: community, onShop: shop }: ExpeditionProps) => {
  ...
  return <section className="exp-card exp-journey" data-action-phase={action.phase}><div className="exp-card-heading"><span className="exp-icon">{r.glyph}</span><div><small>{t.mode === 'idle' ? '已勘测路线 · 挂机远行' : `第 ${t.leg + 1}/${t.route.length} 段 · ${t.paused ? '已在基地暂停' : '与你一起走'}`}</small><h2>{r.name} · {t.mode === 'idle' ? '伙伴正在路上' : t.step === campStep ? r.base : pathLabels[t.step]}</h2></div><span role="status">{busy ? '行动中…' : ''}</span></div>
    {t.mode === 'idle' ? <><div className="exp-timed"><Clock size={28} /><strong>还需 {Math.floor(leftMinutes / 60)} 小时 {leftMinutes % 60} 分</strong><span>已完成 {t.settledParts}/{t.parts} 次采集</span></div><progress aria-label="挂机远行进度" max={t.parts * 3600000} value={Math.max(0, Math.min(t.parts * 3600000, Date.now() - t.startedAt))} /><p>{t.rulesVersion >= 2 ? `目标：${valley ? valleyGatherNames[t.target ?? 'valley_mushroom'] : itemName(r.product)}。已得 ${t.coins} 金币、${t.hearts} 小心心；采集机会仍预留 ${t.reservedHarvests ?? 0} 次。每满一小时结算，召回退回未用机会及未开始时段的配餐与补给费，饱食和体力按实际时长消耗。全部材料运回。` : '每满一小时取得当地常见产物，旧行程仍按出发时的规则结算。'}健康低于 20% 自动返回。</p><div className="exp-stock-list">{Object.entries(t.bag).map(([id, n]) => <p key={id}>{itemName(id)} ×{n}</p>)}</div></> : <>
    ...
    {t.mode === 'idle' && t.rationSegments && <details className="exp-bag"><summary>查看各段餐包与珍宝发现</summary>{t.rationSegments.map((segment, i) => <p key={i}>第 {i + 1} 段 · {segment.settled ? segment.won ? '已完成，找到珍宝 ×1' : '已完成，本段未找到珍宝' : segment.started ? '正在使用' : '尚未开始，可退款'} · 珍宝概率 {segment.chance.toFixed(1)}% · 配餐 {segment.count} 份 · 购买费 {segment.price} 金币</p>)}</details>}
