...
export const ExplorationBudgetCard = ({ pet }: Pick<ExpeditionProps, 'pet'>) => {
  ...
  return <section className="community-card valley-budget"><h3>探索机会与酬谢</h3><div className="valley-budget-values"><span><b>{loop?.available ?? 0}<small> / 24</small></b>可用采集机会</span><span><b>{getPatrolFace(pet) * 3}</b>溪谷每日固定金币</span><span><b>{loop?.vouchers.reduce((sum, v) => { const full = (v.quote ?? v.face) * (v.rewardsVersion === 1 ? .75 : 1); return sum + Math.floor(full) - Math.floor(full * v.paid / 100); }, 0) ?? 0}</b>积存固定金币</span></div>
    <details><summary>查看恢复时间与成长条件</summary><p>每 3 小时恢复 1 次采集机会；{reserved ? `${reserved} 次已为挂机预留；` : ''}{(loop?.available ?? 0) + reserved >= 24 ? '机会已满' : `约 ${Math.floor(minutes / 60)} 小时 ${minutes % 60} 分后恢复下一次`}。手动与挂机共用。</p><p>每日 5 点新增 4 份酬谢，最多积存 3 天；手动完整巡路领取一份，挂机每 2 小时领取一份的 80%。新酬谢的 75% 为固定金币，25% 的期望价值转为金币堆、琥珀和金条，在实际采集时独立判定。每日 22 基础小心心也共用。</p><small>当前阶段 {tier}/3：{tier === 1 ? '完成七段故事并修好基地后，溪谷每日固定金币升至 900，另有随机实物。' : tier === 2 ? `基地升至 2 级、累计使用采集机会 ${loop?.used ?? 0}/80、制作溪光水景后，溪谷每日固定金币升至 1800，另有随机实物。` : '溪谷经营成熟。'} 已发出的旧酬谢保留原规则。</small></details>
  </section>;
};
