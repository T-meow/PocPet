...
export const AdventurePreparation = ({ pet, registry, icons, bag, tool, destination, purpose, onPack, onTool, onDepart, onClose, onUseHomeItem, perform, update }: Props) => {
  ...
  const max = selected?.id === 'trail_rope' ? Math.min(1, amount) : Math.min(capacity, amount);
  ...
  const valid = packed <= capacity && Object.entries(carried).every(([id, n]) => n <= (pet.inventory[id] ?? 0));
  const transferItem = () => {
    if (!selected || !selection || amount < count) return;
    const intoBag = selection.source === 'warehouse';
    if (intoBag && !canPack) return;
    if (intoBag && selected.id !== 'trail_rope' && packed + count > capacity) return;
    perform(() => {
  ...
  return <DialogShell className="storage-modal adventure-preparation" backdropClassName="storage-backdrop adventure-modal-backdrop" labelId="adventure-pack-title" onClose={() => perform(onClose)}>
    ...
    <div className="adventure-pack-columns">
      ...
      <section className="adventure-pack-pane adventure-pack-pane--bag" aria-label={L('本次携带的背包', 'Packed travel bag')}><h3><Backpack size={18} />{L('背包', 'Travel bag')}<small>{packed}/{capacity}</small></h3>
        <div className="adventure-pack-slots" aria-label={L('已占用容量', 'Occupied slots')}>{Array.from({ length: capacity }, (_, i) => <i key={i} data-filled={i < packed} />)}</div>
        {update && <ExplorationBackpackUpgrade pet={pet} update={update} />}
        ...
        <div className="adventure-pack-tool"><strong>{L('独立工具位', 'Tool slot')} · {tool ? 1 : 0}/1</strong>{tool ? tile('trail_rope', 'bag', 1) : <span>{L('未装备探路绳', 'No trail rope equipped')}</span>}<small>探路绳 · {toolDurabilityLabel(pet, 'trail_rope')} · 过绳索通路才扣耐久</small></div>
      </section>
    </div>
    ...
    <footer className="adventure-pack-footer">
      {selected && selection ? <div className="adventure-pack-selection"><div><strong>{selected.displayName}</strong><small>{L('仓库', 'Home')} {warehouse[selected.id] ?? 0} · {L('背包', 'Bag')} {carried[selected.id] ?? 0}</small></div>
        ...
        {(canPack || selection.source === 'bag') && <button className="storage-primary" disabled={!max || selection.source === 'warehouse' && selected.id !== 'trail_rope' && packed + count > capacity} onClick={transferItem}>{selection.source === 'warehouse' ? <ArrowRight size={17} /> : <ArrowLeft size={17} />}{selection.source === 'warehouse' ? L('装入', 'Pack') : L('移回仓库', 'Unpack')} ×{count}</button>}
