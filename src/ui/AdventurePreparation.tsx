...
interface Props extends PreparationResources {
  pet: PetState; bag: Inventory; tool: boolean; destination?: AdventureDestinationId; purpose?: CommunityRoute; embedded?: boolean;
...
export const AdventurePreparation = ({ pet, registry, icons, bag, tool, destination, purpose, onPack, onTool, onDepart, onClose, onUseHomeItem, perform, update, embedded }: Props) => {
  ...
  const contents = <>
    {!embedded && <header className="storage-header"><div className="storage-title"><span className="storage-title-icon"><Backpack /></span><h2 id="adventure-pack-title">{L('出发整备', 'Pack for the trip')}</h2></div><button className="icon-button" onClick={() => perform(onClose)} aria-label={L('关闭整备', 'Close preparation')}><X size={20} /></button></header>}
    <PreparationInventory pet={pet} registry={registry} icons={icons} bag={bag} capacity={capacity} onPack={onPack} onUseHomeItem={onUseHomeItem} perform={perform} tool={{ equipped: tool, onChange: onTool }} bagExtra={<>{destination !== 'tutorial' && <ExplorationCheckPreparation pet={pet} />}{update && <ExplorationBackpackUpgrade pet={pet} update={update} />}</>} />
    <div className="adventure-pack-depart"><div><strong>{destination ? adventureJourneyName(destination, purpose) : L('尚未选择目的地', 'No destination selected')}</strong>{destination && !embedded && <small>{purpose ? adventureJourneyCost(purpose) : destination === 'tutorial' ? L('4 个阶段 · 全程饱食 32、体力 8 · 无需额外道具', '4 stops · 32 hunger, 8 energy in total · No extra items needed') : '预计饱食 −90 · 体力 −50 · 最多采集 2 次'}</small>}{(reason || !valid) && <small className="adventure-blocked">{reason || L('库存已变化，请调整携带选择。', 'Inventory changed. Adjust the selection.')}</small>}</div><button className="adventure-depart-button" disabled={Boolean(reason) || !valid} onClick={() => perform(onDepart)}><Compass size={22} />{L('出发', 'Set out')}</button></div>
  </>;
