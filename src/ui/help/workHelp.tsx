...
import { getPartnerScheduleMasteryNextThreshold } from '../../core/partnerScheduleEffects';
...
export const getWorkOfferHelp = (title: string, p: ReturnType<typeof getPartnerScheduleOfferPreview>): HelpContent => ({
  ...
  details: <><p>基础报酬 {p.baseCoins} 金币 · 经验 {p.baseSkillXp}；全程谢礼 {p.completionCoins} 金币 · 经验 {p.completionSkillXp}。</p>{workHelp.details}</>,
});
export const getWorkHelp = (pet: PetState): HelpContent => {
  ...
  return { ...workHelp, details: <>{workHelp.details}{reward && <p>当前工作全程 {reward.coins.coins} 金币、{reward.coins.skillXp} 经验，含谢礼 {reward.completionCoins} 金币、{reward.completionSkillXp} 经验。</p>}{chance > 0 && <p>{t('ui.partnerSchedule.extraRewardChance', { percent: chance })}</p>}</> };
};
...
export const getSkillHelp = (pet: PetState, id: PartnerScheduleCategory): HelpContent => {
  ...
  return {
    ...
    details: <>{id === 'cooking' && <p>{t('ui.partnerSchedule.kitchenHearts', { percent: getKitchenHeartReward(pet, 'plain_rice').skillBonusPercent })}</p>}{master ? <p>{next ? t('ui.partnerSchedule.mastery.next' + next, { count: skill.masterCompletions, target: next }) : t('ui.partnerSchedule.mastery.complete', { count: skill.masterCompletions })}</p> : <p>满级后完整完成对应工作，可累计大师次数。</p>}{workHelp.details}</>,
  };
};
