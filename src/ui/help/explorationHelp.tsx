...
export const campHelp: HelpContent = {
  title: '营具与休整',
  overview: <><p>完成一次有消耗的行动后，可以使用营具休整。每趟每地区只能休整一次，。</p><p>营具使用一次消耗 1 点耐久；体力和健康只补回本地区尚未恢复的损耗。</p></>,
...
export const getBudgetHelp = (pet: PetState): HelpContent => {
  ...
  return {
    ...
    details: <><p>手动完整地标探索领取一份酬谢，挂机每 2 小时领取一份的 80%。每日 22 基础心心也共用。</p>{commonTreasureRules}</>,
  };
};
...
export const treasureFindingHelp: HelpContent = {
  title: '寻找珍宝',
  overview: <><p>当地探索可以调查珍宝，调查进度跨天保留。工具可以帮助调查，行动前会列出实际消耗。</p><p>挂机每两小时寻找一次当地珍宝，同地区连续 9 次未得后，第 10 次必得；随机获得清零，跨日保留。金币堆、琥珀与金条在各地采集中也有机会发现。</p></>,
