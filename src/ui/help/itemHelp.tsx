...
const itemRules: Partial<Record<string, string>> = {
  ...
  survey_lens: '保留旧调查用途；当前地标采集不单列放大镜操作，也不会自动消耗。食材调查会从普通采集的当地奖池抽取。',
  trail_rope: '仓库有绳索即可选择对应通路，不需装入行囊；使用时扣耐久，用尽扣一件。旧行程已携带的绳索优先使用，剩余部分返程归还。危险行动中可减轻擦伤。',
