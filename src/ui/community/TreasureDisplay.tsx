...
export const TreasureDisplay = ({ pet, update }: Pick<CommunityPanelProps, 'pet' | 'update'>) => <section className="community-card community-treasures">
  ...
  <div className="community-treasure-grid">{communityDecorationIds.map(id => {
    ...
    return <article key={id} data-built={built}>
      ...
      <div><b>{built ? '✦ 已陈列 · ' : ''}{d.name}</b><small>{d.description}</small>
        {built ? <small>永久装饰 · 同类珍宝仍可出售</small> : <><small>{Object.entries(d.items).map(([item, count]) => `${getInventoryItem(item as ItemId)?.name} ${pet.inventory[item] ?? 0}/${count}`).join(' · ')}</small>
          ...
          <button className="secondary-button" disabled={!ready || pet.timePause !== undefined || !canSpendCompanionTime(pet)} onClick={() => update(p => buildCommunityDecoration(p, id))}>制作{d.name}</button></>}
      </div>
    </article>;
  })}</div>
</section>;
