...
export const CommunityMarket = ({ pet, update, onShop, registry, itemIconMap }: CommunityPanelProps) => {
  ...
  return <>
    <section className="community-production-scene community-market-scene" data-production-scene="market" data-open={m.open} aria-label="溪畔小摊场景">
      ...
      <footer className="community-production-footer"><div className="community-production-caption"><span>累计售出 {m.sold} 份 · 收入 {m.revenue} 金币</span><span>{m.open ? '低价货成交快，高价货慢慢卖，偶尔还有慷慨大单' : '准备好货品，再开店迎接邻居'}</span></div><div className="community-production-dock">
