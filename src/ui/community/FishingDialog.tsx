...
import { ArrowLeft, Crown, Fish, Settings2, Utensils, X, Zap } from 'lucide-react';
...
export const FishingDialog = ({ pet, update, portrait, actorId, actorName, onClose, onShop, onManage, initialWater = 'pond', itemIconMap }: CommunityPanelProps & {
  ...
  const [strong, setStrong] = useState(!(pet.inventory.fishing_rod ?? 0));
  ...
  return <DialogShell className="fishing-dialog" backdropClassName="fishing-modal-backdrop" labelId="fishing-dialog-title" onClose={onClose}>
    <header className="fishing-dialog-header"><h2 id="fishing-dialog-title">{waters[selectedWater].name}</h2><button type="button" className="fishing-close" aria-label="关闭钓鱼窗口，保留当前进度" onClick={onClose}><X size={22} /></button></header>
    {view !== 'scene' && preparing ? <div className="fishing-settings">
      ...
      {view === 'gear' ? <>
        ...
        <h3>带一点好吃的</h3><p className="fishing-note">{hours} 小时需要至少 {quote.food.minimum} 份、{quote.food.nutrition} 点基础饱食，最多 {quote.food.maximum} 份。已备 {quote.food.count} 份、{quote.food.hunger} 点。</p>
        <label className="fishing-autofill"><input type="checkbox" checked={rations.autoFill} onChange={e => setRations({ ...rations, autoFill: e.target.checked })} />自动补给 · {standardRationPrice} 金币／份</label><p className="fishing-note">自动补足 {quote.food.purchased} 份，共 {quote.food.coins} 金币。</p>
        ...
        <p className="fishing-note">食物覆盖挂机期间的饱食和体力消耗。提前返回会先吃剩余食物，吃不完分给邻居，食物和补给费不退。</p>{quote.food.reason && <p className="fishing-warning" role="status">{quote.food.reason}</p>}
        <button className="fishing-text-button" onClick={onShop}>去商店补充食物</button>
      </>}
      <button className="fishing-primary" onClick={() => setView('scene')}>准备好了</button>
    </div> : <div className="fishing-play">
      ...
      <div className="fishing-picture"><FishingSceneArt water={selectedWater} portrait={portrait} name={actorName} phase={phase} reelTick={manual?.clicks} />{pending?.mode === 'manual' && catches[0] && <div className="fishing-catch-hero"><img src={icon(catches[0].fish)} alt={fish[catches[0].fish].name} />{catches[0].crown && <span><Crown size={22} />首次金冠</span>}</div>}</div>
      ...
      {preparing && mode === 'idle' && <div className="fishing-idle-setup"><div className="fishing-durations" role="group" aria-label="挂机时长">{[2, 4, 8].map(n => <button aria-pressed={hours === n} key={n} onClick={() => setHours(n)}>{n} 小时</button>)}</div><button className="fishing-food-summary" onClick={() => setView('food')}><Utensils size={18} /><span>食物 {quote.food.count} 份 · 自动补给 {quote.food.coins} 金币<small>准备食物</small></span></button><label className="fishing-autofill"><input type="checkbox" checked={rations.autoFill} onChange={e => setRations({ ...rations, autoFill: e.target.checked })} />自动补足食物 · {standardRationPrice} 金币／份</label></div>}
      <footer className="fishing-action">
        <p role="status">{frozen ? '时间已冻结，恢复后继续' : preparing && mode === 'manual' && !reason ? <span className="fishing-cost">
