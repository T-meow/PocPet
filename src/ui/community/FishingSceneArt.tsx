...
export const FishingSceneArt = ({ water, portrait, name, phase, reelTick = 0 }: {
  water: WaterId; portrait?: string; name: string; phase: FishingScenePhase; reelTick?: number;
}) => {
  ...
  return <div className="fishing-scene" data-water={water} data-phase={phase} role="img" aria-label={`${name}在水边钓鱼${phase === 'biting' ? '，鱼儿上钩了' : ''}`}>
    ...
    {portrait ? <img className="fishing-scene-actor" src={portrait} alt="" /> : <svg className="fishing-scene-actor" viewBox="0 0 160 200" aria-hidden="true"><ellipse cx="80" cy="147" rx="48" ry="48" fill="#efe0c3" /><circle cx="80" cy="75" r="48" fill="#f5e7ce" /><path d="M39 45L27 2L66 31M95 30L131 2L123 52" fill="#e2b898" /><circle cx="63" cy="77" r="4" fill="#756453" /><circle cx="99" cy="77" r="4" fill="#756453" /><path d="M73 94q9 8 18 0" fill="none" stroke="#756453" strokeWidth="3" /></svg>}
    <svg viewBox="0 0 600 360" aria-hidden="true" className="fishing-scene-layer fishing-scene-front">
      ...
      <g className="fishing-rod-motion" key={reelTick}><path d="M186 203Q260 65 400 159" fill="none" stroke="#88694d" strokeWidth="5" strokeLinecap="round" /><path d="M400 159L437 246" fill="none" stroke="#f8f5df" strokeWidth="1.7" /></g>
