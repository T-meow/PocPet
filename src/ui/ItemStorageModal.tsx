...
import { durableToolIds, type DurableToolId } from '../core/fieldEquipmentData';
...
export const ItemStorageModal = ({ mode, pet, items, itemIconMap, browse, onBrowseChange, onClose, onSwitch, context, quantityDisabled, footer, tileInfo, renderActions, favoriteFoodIds }: Props) => {
  ...
  {isDetailOpen && item && <DialogShell className={`storage-modal storage-modal--${mode} storage-item-modal`} backdropClassName={backdropClassName} labelId="storage-item-title" onClose={closeDetail}>
    ...
    <section className="storage-detail" data-tone={getItemBrowseTone(item)}>
        <div className="storage-detail-copy"><div className="storage-detail-hero"><div className="storage-detail-art"><img src={iconFor(item)} alt="" draggable={false} /></div><div><h3>{item.displayName}</h3><span className="storage-category-label">{isKitchenIngredient(item) ? L('厨房食材', 'Cooking ingredient') : categories.find((category) => category.id === item.kind)?.label}</span></div></div><p className="storage-detail-description">{item.displaySummary}</p>{durableToolIds.includes(item.id as DurableToolId) && <p className="storage-ingredient-note">耐久 · {toolDurabilityLabel(pet, item.id as DurableToolId, item.id === 'trail_rope' && Boolean(context && (pet.adventure.active?.tool || pet.community.expedition.active?.tool)))} · 用尽消耗一件，备用工具保持满耐久</p>}{effects.length > 0 && <><p className="storage-effects-title">{L('每份基础效果', 'Base effects per item')}</p><div className="storage-effects">{effects.map((effect) => <span key={effect.key}>{effect.label}</span>)}</div></>}{!context && isKitchenIngredient(item) && <p className="storage-ingredient-note">{item.usable ? L('可以喂给伙伴，也可以留着做菜。两处显示的是同一份库存。', 'Feed your companion or save it for cooking. Both categories share this stock.') : L('留给厨房的原料，选一道菜就能派上用场。', 'An ingredient for the kitchen. Pick a recipe to use it.')}</p>}{context?.note && <p className="storage-ingredient-note">{context.note}</p>}</div>
