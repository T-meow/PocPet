...
export const AdventureMap = ({ adventure, today, selection, portrait, landscape, onToggleLandscape, onSelect, onPrepare, onResume, onCollect, onClose }: Props) => {
  ...
  const region = regions.find(entry => entry.id === selection.region)!;
  const names = getAdventureMapNames(region.id);
  const theme = adventureMapThemes[region.id];
  const status = getAdventureNodeStatus(adventure, region.id, selection.node, today);
  const entry = selection.node === 'entrance';
  const scene = getAdventureNodeScene(region.id, selection.node);
  ...
  const nodeHint = !region.open ? L('这片地区还在筹备中。可以先看看地标，等下一次远行。', 'This region is coming later. Browse its landmarks while you wait.')
    : entry ? L('先熟悉入口附近的路，再向地图深处出发。溪谷六段探查每天可完成一次，凌晨 5 点刷新；未完成可返程重试。', 'Learn the entrance paths before heading deeper. Complete six steps once daily, resetting at 5 a.m.; unfinished trips can be retried.')
      : selection.node === 'encounter' ? L('后续在这里遇见特殊来客，可以用食物引诱、绕路或其他方式应对。遭遇任务筹备中。', 'Future encounters offer food lures, detours and other choices. This task is coming later.')
        : selection.node === 'camp' ? L('地图深处的落脚点，等后续旅程一起发现。营地任务筹备中。', 'A resting place deeper in the region awaits a future journey. Camp tasks are coming later.')
  ...
  return <DialogShell className="adventure-dialog adventure-map-dialog" backdropClassName="adventure-modal-backdrop" labelId="adventure-map-title" onClose={onClose}>
    ...
    <div className="adventure-map-body" style={{ '--map-color': theme.color, '--map-pale': theme.pale } as CSSProperties}>
      <nav className="adventure-map-regions" aria-label={L('选择大地图', 'Choose a region')}>
        {regions.map((item, i) => { const Icon = regionIcons[item.id]; return <button key={item.id} onClick={() => onSelect({ region: item.id, node: 'entrance' })} aria-pressed={item.id === region.id} style={{ '--tab-color': adventureMapThemes[item.id].color, '--tab-pale': adventureMapThemes[item.id].pale } as CSSProperties}>
      ...
      <div className="adventure-map-layout">
          ...
          <div className="adventure-map-frame"><div className="adventure-map-canvas" data-region={region.id}>
            ...
            {adventureMapNodes.map(node => {
              ...
              return <button className="adventure-map-node" key={node.id} data-node={node.id} data-status={state} aria-pressed={node.id === selection.node} aria-label={names[node.id] + ' · ' + adventureNodeStatusLabel(state)} style={{ left: node.x + '%', top: node.y + '%' }} onClick={() => onSelect({ region: region.id, node: node.id })}>
                <span className="adventure-map-pin"><Icon size={20} />{state === 'complete' ? <Check className="adventure-map-node-mark" size={12} /> : state === 'planned' ? <LockKeyhole className="adventure-map-node-mark" size={11} /> : null}</span>
        ...
        <section className="adventure-map-detail" aria-label={L('节点任务详情', 'Landmark task details')} aria-live="polite" data-selected-node={selection.node}>
          <div className="adventure-map-detail-heading"><span className="adventure-map-status" data-status={status}>{status === 'planned' ? <LockKeyhole size={14} /> : status === 'complete' ? <Check size={14} /> : <Leaf size={14} />}{adventureNodeStatusLabel(status)}</span><small>{region.name}</small></div>
          <h4>{names[selection.node]}</h4>
          {scene && <img className="adventure-map-scene-preview" src={scene} alt={names[selection.node] + L('场景', ' scene')} />}
          ...
          <div className="adventure-map-detail-actions">
            {adventure.active ? <button className="primary-button" onClick={onResume}><ArrowRight size={17} />{L('继续当前探查', 'Resume current scouting')}</button>
              : adventure.pending ? <button className="primary-button" onClick={onCollect}><Backpack size={17} />{L('先领取上次行囊', 'Collect your previous bag')}</button>
                : <button className="primary-button" disabled={!canPrepare} onClick={canPrepare ? onPrepare : undefined}><Backpack size={17} />{canPrepare ? L('进入节点 · 整备出发', 'Enter landmark · Pack to leave') : status === 'complete' ? region.id === 'valley' ? L('今日已完成 · 明日再来', 'Done today · Return tomorrow') : L('入口已完成', 'Entrance complete') : L('任务筹备中', 'Task coming later')}</button>}
