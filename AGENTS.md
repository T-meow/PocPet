...
- 日常自动检查只有 `npm.cmd test`：TypeScript、存档兼容／往返／恢复、异常处理和入口模块加载。代码改动按需要运行一次；纯文档、文案和样式调整不例行跑全套测试或构建。
- 玩法节奏、数值、界面和视觉效果优先由用户人工检查。默认不新增玩法专项自动化脚本；确需保留的存档或报错检查合并进 `scripts/check.mjs`，不恢复逐功能测试矩阵。
- 打包所需的版本、架构、资源和产物校验也共用 `scripts/check.mjs`，仅在打包／发布流程调用对应模式；不因本地功能修改自动打包。
...
- README 保留简介、使用入口与许可；`docs/` 只保留 Wiki、架构／格式说明、当前数值表、操作参考和素材规范／来源清单。
- 不新增或保留已完成的实施记录、测试流水账、美术批次日志、未来规划与路线图；长期有效的约束合并到现有参考文档，历史实现从 Git 查阅。
- 不新增或保留含本机绝对路径、个人用户目录等隐私信息的素材清单或开发记录；公开文档仅使用项目相对路径或通用占位符。
