...
外部存档由 `saveCodec.ts` 生成 JSON v2，最低读取版本为 1.9.0；只包含 `persistedPet.ts` 白名单状态及当前 Mod 摘要，不包含 Mod 图片。从 2.1.0 起，读取兼容下限为 1.9.0，保留普通与压缩 JSON v2；拒绝更早的裸 JSON、v1 封装、`POCPET-SAVE-v2:` 保护文本，以及 `minimumReaderVersion` 低于 1.9.0 的 v2 存档。过旧存档明确提示并保留原文，恢复列表跳过过旧备份，不阻塞其余有效恢复点。

导入时会重置离线时间、能量恢复、睡眠和番茄钟等时间基线，避免恢复旧备份后立即产生大量离线结算。来自更高 schema 或未知冒险规则版本的存档应明确拒绝并提示升级。已由 1.9 或之后版本保存的旧进度继续保留身份、已领取标记与待领补偿。
...
- 摆摊：基础售价和材料计价统一由 `communityEconomy.ts` 计算，静态价值表只作基准；每道加工／料理工序按计价成本加 25%，摆摊与招牌加成另计。采购和自产共用库存及价格，社区不提供即时回收。规则和当前价格见[食材与料理数值表](食材与料理数值表.md)。
- 摆摊成交：`communityMarketRules.ts` 以整批基础总价控制成交概率，常规／大单／包场的基础期望额度为 20／200／6000，成功仍按原价整批出售。烹饪 Lv.2–10 对包场每级 +5%，铭牌 Lv.1–10 每级 +5%，两项相加后乘入包场额度，最高 +95%；普通交易和大单不受此加成影响。一次到访只判定一次，成功后统一扣货记账；判定使用独立的第 32 号随机抽样，离线分段、刷新及恢复不会重抽。普通购买 1–3 份，慷慨游客仍有 8–20 份大单和低概率包场；扩建不增加客流，报价加成不减慢成交。
- 货架迁移：`market.pricingVersion` 当前为 1，旧档缺省为 0。载入先按旧挂牌价结算离线成交，再重估未售货品并更新报价修订号；暂停存档只迁移价格。导入沿用时间重定位规则，重复恢复不重复调价、不补发历史收入。完成迁移后上架和补货保留栏位原报价。
...
- 模块版本为 adventure 8、community 13、expedition 5。压缩保存保留新进度、委托目标、保底与待领余额；`scripts/check.mjs` 覆盖迁移、全图物品往返和重复恢复。满仓和满币都保留剩余待领成果。
...
角色制作与打包流程见[角色 Mod 制作指南](Mod制作指南.md)。`favoriteFoodIds` 支持基础物品白名单和厨房已注册成品料理；料理偏好不扩展物品图片或文案覆盖白名单。

代码侧主要边界：

- `mod.ts` 负责白名单路径、manifest 字段、图片大小和自定义道具验证。
- `modStorage.ts` 最多保留 12 个外部 Mod；内置角色不占导入名额，外部图片 Blob 存 IndexedDB。
- 内置 Doro、Mint、DeepSeek 与外部 Mod 共用可用角色列表；Doro 和 DeepSeek 的删除标记保存在 Mod 库，Mint 受保护。删除当前角色切换到 Mint，同时更新存档角色标识；Furo 仍为基础角色入口。
- 大厅邻居和新探险偶遇使用可用角色库。已开始行程保留角色 ID，读取存档时允许有效的外部 Mod ID，不依赖角色是否仍安装。
...
打包参数、平台范围、产物命名和签名见[打包参考](打包流程与统一脚本.md)，发布授权与正式标签规则见 [AGENTS.md](../AGENTS.md)。本地修改不自动推送、打包或发布。
