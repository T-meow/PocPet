...
- 所有当前 BGM 的最终 MP3 综合响度为 −18 LUFS（容差 0.3 LU），真峰值不高于 −1 dBTP；保留时长和循环边界。源文件及最终测量值见音频素材清单。BGM 默认音量 60%，音量与后台开关按本机保存。
...
- 只有实际播放且未静音的鉴赏时间累计奖励；每 600 秒固定 1 小心心，无每日／会话上限，不受等级和奖励加成影响。结束时一次性领取，余量跨天、跨会话保留。时间冻结期间可听歌但不累计或领取奖励。
